...
$consulta_rastreio = "SELECT `id`, `numero_rastreio`, `status`, `data_criacao`, `id_usuario`, `email`, `auto_update` FROM `rastreios` WHERE numero_rastreio = '$nmr_rastreio'";
...
if($linhas == 0){
    $sql_insert = "INSERT INTO `rastreios` (`numero_rastreio`, `status`, `data_criacao`, `id_usuario`, `email`, `auto_update`) VALUES ('$nmr_rastreio', '$status', '$date', '$id_usr', '$email', '1')";

    if (mysqli_query($conn, $sql_insert)){
        echo "<script>alert('Registro criado com sucesso!');</script>";
        echo "Redirecionando para pagina anterior...";
        header('Refresh: 0.5; URL=rastreios.php');
    } else {
    ...
    if($dados['status'] != $status){
        ...
        echo "Redirecionando para pagina anterior...";
        header('Refresh: 0.5; URL=gerar_rastreio_adv.php');
    }
}
